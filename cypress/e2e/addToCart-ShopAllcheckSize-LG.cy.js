...
// on sale
const pdps = ["/products/reptile-stripe-mesh-bralette","/products/reptile-stripe-mesh-brief"]
...
const iconCard = '.nav-animahdr22 .eby-mobile-nav .cart-link.jsDrawerOpenRight';
...
// for( let i = 11; i < 12; i++){
    ...
    describe(`${i}. Start of test for href ${pdps[i]} and check size LG is available`, () => {
        ...
        // it(`Opens caricon `, () => {
            cy.wait(1000);
            cy.get('.drawerProductTitle > span').should(el => {
                
                expect( (el).eq(0)).to.have.text(newSize)
            })
            cy.get(iconCard).click()
        // })   
        ...
        // it(`Opens checkout`, () => {
            ...
            cy.get('.drawerButtonBox').click()

            cy.wait(3000);
            cy.get('.order-summary-toggle__icon-wrapper').click()
            cy.get('.product__description__variant.order-summary__small-text')
                .should('have.text', newSize)
        })       
